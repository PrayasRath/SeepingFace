# -*- coding: utf-8 -*-
"""
Created on Wed Jun  9 13:11:15 2021
...
wdir = os.path.join(r'E:\seepage_face_UZF2','SLF_SFF','diff_width_hi_slope')
model_name_fmt = 'rel{0:3.2f}_vs_width{1:3.2f}'

slope=0.1 
vdepth = 50.
width_list = np.array([0.01,0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.4,0.45,0.5])
...
for i in range (len(width_list)):
    ...
    for j in range (len(rel_lev)):
        ...
        model_name = model_name_fmt.format(rel_lev[j],width_list[i])
        mdir = os.path.join(wdir,model_name)

        if not os.path.isdir(mdir):
            os.makedirs(mdir)

        in_dict={'slope':0.1,'depth':50,'width':L*width_list[i],
         'rel_lev':rel_lev[j],'aq_thick':aq_thick,
...
os.chdir(wdir)
np.savetxt("uzf_diff_width_SLF_hi_s.csv",SLF_list , delimiter=",", fmt='%s')
np.savetxt("uzf_diff_width_SFF_hi_s.csv",SFF_list , delimiter=",", fmt='%s')
np.savetxt("uzf_diff_width_SLF_regional_hi_s.csv",SLF_regional_list , delimiter=",", fmt='%s')
np.savetxt("uzf_diff_width_SFF_regional_hi_s.csv",SFF_regional_list , delimiter=",", fmt='%s')